...
@Component({
  ...
  template: `
<div class="page-shell clase2-page">
  @if (loading()) {
    <div class="loading-state">
      <div class="spinner"></div>
      <p>Cargando Clase 2…</p>
    </div>
  } @else if (error()) {
    ...
    <app-page-back-bar backLink="/clase/2" backLabel="Volver a Clase 2" [breadcrumb]="[
        { label: 'Inicio', link: '/' },
        { label: 'Clase 2', link: '/clase/2' },
    ...
    @if (data()) {
      <header class="page-hero">
        ...
        <div class="flex gap-3 mt-3">
          <a [routerLink]="data().rutas_relacionadas?.unidad_completa || '/modulo/1/unidad/1'"
      ...
      <section class="section">
        ...
        <div class="content-grid">
          @for (tema of data().temas; track tema.id) {
            <a [routerLink]="['/clase/2/tema', tema.id]" class="card card--bordered theme-card">
    ...
    @if (temaDetallado()) {
      <section class="section tema-section">
            ...
            <div class="content-grid">
              @for (c of temaDetallado().detalle.conceptos_clave; track c.nombre) {
                <div class="formula-card card card--bordered">
                  ...
                  @if (c.bandas) {
                    <p class="formula-card__desc mt-2"><strong>Distribución de bandas:</strong> {{ c.bandas }}</p>
                  }
                  @if (c.polaridad) {
                    <p class="formula-card__desc mt-2"><strong>Polaridad:</strong> {{ c.polaridad }}</p>
                  }
                  @if (c.aplicacion) {
                    <p class="formula-card__desc mt-2"><strong>Aplicación:</strong> {{ c.aplicacion }}</p>
                  }
                  @if (c.tipos?.length) {
                    <div class="mt-2 text-secondary font-sm">
                      <strong>Tipos comunes:</strong>
                      <ul class="bullet-list pl-3">
                        <li *ngFor="let t of c.tipos">{{ t }}</li>
                      </ul>
                    </div>
                  }
                  @if (c.modos?.length) {
                    <div class="mt-2 text-secondary font-sm">
                      ...
                      <ul class="bullet-list pl-3">
                        <li *ngFor="let m of c.modos">{{ m }}</li>
                      </ul>
                    </div>
                  }
                  @if (c.ideal?.length) {
                    <div class="mt-2 text-secondary font-sm">
                      <strong>Características ideales:</strong>
                      <ul class="bullet-list pl-3">
                        <li *ngFor="let id of c.ideal">{{ id }}</li>
                      </ul>
                    </div>
                  }
                  @if (c.funciones?.length) {
                    <div class="mt-2 text-secondary font-sm">
                      <strong>Funciones:</strong>
                      <ul class="bullet-list pl-3">
                        <li *ngFor="let fn of c.funciones">{{ fn }}</li>
                      </ul>
                    </div>
                  }
                  @if (c.ajustes?.length) {
                    <div class="mt-2 text-secondary font-sm">
                      ...
                      <ul class="bullet-list pl-3">
                        <li *ngFor="let aj of c.ajustes">{{ aj }}</li>
                      </ul>
                    </div>
                  }
                  @if (c.modos?.length) {
                    <div class="mt-2 text-secondary font-sm">
                      <strong>Modos de acoplamiento:</strong>
                      <ul class="bullet-list pl-3">
                        <li *ngFor="let m of c.modos">{{ m }}</li>
                      </ul>
                    </div>
            ...
            <div class="content-grid content-grid--3">
              @for (f of temaDetallado().detalle.formulas; track f.titulo) {
                <div class="formula-card card card--bordered">
                  ...
                  @if (f.derivacion) {
                    <p class="text-secondary font-sm mt-2"><em>{{ f.derivacion }}</em></p>
                  }
                </div>
              }
            </div>
        ...
        <!-- Gráfica interactiva -->
        @if (temaDetallado().id === 'componentes-pasivos') {
          <section class="section">
            <h3 class="section__title">🔌 Visualización Interactiva</h3>
            <div class="content-panel">
              <app-equation-chart 
                chartType="capacitor_reactance"
                title="Reactancia Capacitiva: Oposición vs Frecuencia"
                equation="X_C = \\frac{1}{2\\pi f C}">
              </app-equation-chart>
            </div>
          </section>
        } @else if (temaDetallado().id === 'componentes-activos') {
          <section class="section">
            <h3 class="section__title">🔌 Visualización Interactiva</h3>
            <div class="content-panel font-sm">
              <app-equation-chart 
                chartType="diode_iv"
                title="Curva Característica I-V del Diodo de Silicio"
                equation="I_D = I_S (e^{\\frac{V_D}{V_T}} - 1)">
              </app-equation-chart>
            </div>
          </section>
        } @else if (temaDetallado().id === 'instrumentos-medicion') {
          <section class="section">
            <h3 class="section__title">🔌 Visualización Interactiva</h3>
            <div class="content-panel">
              <app-equation-chart 
                chartType="ohm"
                title="Monitoreo de voltaje y corriente"
                equation="V = I \\cdot R">
              </app-equation-chart>
            </div>
          </section>
        ...
        @if (temaDetallado().detalle?.secciones?.length) {
          @for (sec of temaDetallado().detalle.secciones; track sec.id) {
            <section class="section">
              ...
              <div class="content-panel text-body">
                <p>{{ sec.contenido }}</p>
              </div>
            </section>
          }
        ...
        <footer class="page-footer-nav">
          <app-page-back-bar backLink="/clase/2" backLabel="Volver a Clase 2" />
...
export class Clase2Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe((params) => {
      ...
      if (temaId) {
        this.http.get<any>(`/assets/data/clase-2/${temaId}.json`).subscribe({
          next: (detalle) => {
        ...
        this.http.get<any>('/assets/data/clase-2/index.json').subscribe({
          next: (d) => {
            this.data.set(d);
          ...
          error: () => {
            this.error.set('No se pudo cargar el índice de la Clase 2.');
            this.loading.set(false);
          }
        });
      }
