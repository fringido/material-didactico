...
@Component({
  ...
  template: `
<div class="page-shell clase1-page">
  @if (loading()) {
    <div class="loading-state">
      <div class="spinner"></div>
      <p>Cargando Clase 1…</p>
    </div>
  } @else if (error()) {
    ...
    <app-page-back-bar backLink="/clase/1" backLabel="Volver a Clase 1" [breadcrumb]="[
        { label: 'Inicio', link: '/' },
        { label: 'Clase 1', link: '/clase/1' },
      ...
      <section class="section">
        ...
        <div class="content-grid">
          @for (tema of data().temas; track tema.id) {
            <a [routerLink]="['/clase/1/tema', tema.id]" class="card card--bordered theme-card">
    ...
    @if (temaDetallado()) {
      <section class="section tema-section">
                ...
                <div class="formula-card card card--bordered">
                  ...
                  @if (c.ejemplos?.length) {
                    <div class="mt-2 text-secondary font-sm">
                      ...
                      <ul class="bullet-list pl-3">
                        <li *ngFor="let ex of c.ejemplos">{{ ex }}</li>
                      </ul>
                    </div>
                  }
                  @if (c.caracteristicas?.length) {
                    <div class="mt-2 text-secondary font-sm">
                      ...
                      <ul class="bullet-list pl-3">
                        <li *ngFor="let ch of c.caracteristicas">{{ ch }}</li>
                      </ul>
                    </div>
                  }
                  @if (c.diferencias_clave) {
                    <p class="formula-card__desc mt-2"><strong>Diferencias:</strong> {{ c.diferencias_clave }}</p>
                  }
                  @if (c.unidad) {
                    <p class="formula-card__desc mt-2"><strong>Unidad:</strong> {{ c.unidad }}</p>
                  }
                  @if (c.analogia) {
                    <p class="formula-card__desc mt-2"><strong>Analogía:</strong> {{ c.analogia }}</p>
                  }
                </div>
              }
            </div>
        ...
        @if (temaDetallado().detalle?.grafica_interactiva) {
          <section class="section">
            <h3 class="section__title">🔌 Visualización Interactiva</h3>
            <div class="content-panel">
              <app-equation-chart 
                [chartType]="temaDetallado().detalle.grafica_interactiva.chartType"
                [title]="temaDetallado().detalle.grafica_interactiva.title"
                [equation]="temaDetallado().detalle.grafica_interactiva.equation">
              </app-equation-chart>
            </div>
          </section>
        } @else if (temaDetallado().id === 'conceptos-basicos') {
          <section class="section">
            <h3 class="section__title">🔌 Visualización Interactiva</h3>
            <div class="content-panel">
              <app-equation-chart 
                chartType="ohm"
                title="Ley de Ohm: Voltaje vs Corriente"
                equation="V = I \cdot R">
              </app-equation-chart>
            </div>
          </section>
        ...
        <footer class="page-footer-nav">
          <app-page-back-bar backLink="/clase/1" backLabel="Volver a Clase 1" />
...
export class Clase1Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe((params) => {
      ...
      if (temaId) {
        this.http.get<any>(`/assets/data/clase-1/${temaId}.json`).subscribe({
          next: (detalle) => {
        ...
        this.http.get<any>('/assets/data/clase-1/index.json').subscribe({
          next: (d) => {
            this.data.set(d);
          ...
          error: () => {
            this.error.set('No se pudo cargar el índice de la Clase 1.');
            this.loading.set(false);
          }
        });
      }
