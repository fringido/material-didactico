...
@Component({
  ...
  template: `
<div class="page-shell clase6-page">
  @if (loading()) {
    <div class="loading-state">
      <div class="spinner"></div>
      <p>Cargando Clase 6…</p>
    </div>
  } @else if (error()) {
    ...
    <app-page-back-bar backLink="/clase/6" backLabel="Volver a Clase 6" [breadcrumb]="[
        { label: 'Inicio', link: '/' },
        { label: 'Clase 6', link: '/clase/6' },
    ...
    @if (data()) {
      <header class="page-hero">
        ...
        <div class="flex gap-3 mt-3">
          ...
          @if (data().rutas_relacionadas?.clase_anterior) {
            <a [routerLink]="data().rutas_relacionadas.clase_anterior" class="btn btn--ghost btn--sm">
              ← Clase Anterior
            </a>
          }
        </div>
      ...
      <section class="section">
        ...
        <div class="content-grid">
          @for (tema of data().temas; track tema.id) {
            <a [routerLink]="['/clase/6/tema', tema.id]" class="card card--bordered theme-card">
    ...
    @if (temaDetallado()) {
      <section class="section tema-section">
        ...
        <!-- SIMULADORES Y GRÁFICAS DINÁMICAS -->
        @if (temaDetallado().detalle?.simulacion_dinamica) {
          <section class="section">
            <h3 class="section__title">Laboratorio Interactivo</h3>
            <app-dynamic-circuit-simulator [simulacion]="temaDetallado().detalle.simulacion_dinamica">
            </app-dynamic-circuit-simulator>
          </section>
        }

        @if (temaDetallado().id === 'respuesta-frecuencia') {
          <section class="section">
            <h3 class="section__title">Analizador de Bode Interactivo</h3>
            <app-bode-chart></app-bode-chart>
          </section>
        }
        ...
        <footer class="page-footer-nav">
          <app-page-back-bar backLink="/clase/6" backLabel="Volver a Clase 6" />
...
export class Clase6Component implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe((params) => {
      ...
      if (temaId) {
        // Carga desde la unidad unificada
        this.http.get<any>('/assets/data/modulo-1/unidad-3.json').subscribe({
          next: (data) => {
            const tema = data.temas.find((t: any) => t.id === temaId);
            if (tema) {
              this.temaDetallado.set({ 
                ...tema, 
                detalle: tema.contenido 
              });
            } else {
              this.error.set(`No se encontró el tema "${temaId}".`);
            }
            this.data.set(null);
          ...
          error: (err) => {
            ...
            this.error.set(`No se pudo cargar la unidad.`);
            this.loading.set(false);
          }
        });
      } else {
        // Carga la información de la unidad como si fuera la clase
        this.http.get<any>('/assets/data/modulo-1/unidad-3.json').subscribe({
          next: (d) => {
            this.data.set({
              ...d,
              metadata: {
                ...d.metadata,
                clase: 6 // Mantenemos la etiqueta de clase para el usuario
              },
              rutas_relacionadas: {
                unidad_completa: '/modulo/1/unidad/3',
                clase_anterior: '/clase/5'
              }
            });
          ...
          error: () => {
            this.error.set('No se pudo cargar el índice.');
            this.loading.set(false);
          }
        });
      }
